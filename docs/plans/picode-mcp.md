...
| **N1 — inbox and checklist** (landed 2026-09-19: `picode mcp inbox checklist`, cards, launch group; `ask_human` waits on the item) | `inbox` and `checklist` families; the terminal card shows a guest's plan line | Per-family tests; an `ask_human` from Claude Code appears in the Inbox and the answer returns | Tests the Inbox |
| **N2 — the other guests** (measured 2026-09-19) | Grok, Hermes, Muse and Antigravity offer no per-launch mechanism (`--help` of the installed binaries and the vendor docs): file scopes only, which the pane and the Launch settings guard already say. Omp (`-e <extension>`) could take pi's packages per launch; not built | — | Decided: document; Omp `-e` only on demand |
