...
| omp | `.omp`, `.agents`, `.agent` (walk-up), `.claude`, `.github` | `~/.omp/agent/skills`, `~/.agents/skills`, `~/.agent/skills` (measured) | `skills.ignoredSkills` (globs; the project list replaces the user list, measured 2026-09-24) | `--skills=<globs>`; `--no-skills` |
...
| codex | `.agents/skills` | `~/.agents/skills` | `[[skills.config]]` keyed by the absolute `SKILL.md` path, user file only (measured 2026-09-24) | `-c 'skills.config=[…]'` filters per run (measured); no way to add a folder |
| grok | `.grok`, `.agents`, `.claude` (trusted projects only, measured) | `~/.grok/skills`, `~/.agents/skills`, `~/.claude/skills` | `[skills] disabled` (TOML list), user file only (measured 2026-09-24) | measure |
...
| launch | isolated agent (Pi, Omp) | Pi: `--no-skills` plus `--skill` per own skill; Omp: an overlay with every folder source off (its `--no-skills` also drops custom folders) |
