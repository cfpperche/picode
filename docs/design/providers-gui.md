...
| **P3** | **Usage** dialog for OAuth plans (5h / 7d / week / extra). `GET /api/providers/{id}/usage`. Button only when `quotaKind` matches the active method (ADR-0030) | API-key meters (ZAI, OpenCode Go); per-vault-account fetch without Use; statusbar quotas |
