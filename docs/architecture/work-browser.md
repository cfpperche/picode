...
| When the human's view moves | only `open` and the command that creates the split select the session; every other verb runs in the split where it is, even off screen. A screenshot of a split off screen waits 4 s for pixels, then reveals it once and retries | `sessionReveal` + `browserChannel.js` (ADR-0172); the `events` verb's raw output carries a `session` receipt — host, reveal, hostOpen, splits — so a wrong bind is read, not guessed |
