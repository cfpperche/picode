...
scoping (an owner-drawn Canvas edge is its one exception, ADR-0116), request
...
**Contacts are the union of the workspace and live Canvas edges
...
Canvas links to it — resolved through the far panel's `(kind, ref)`, deduped,
...
one, through `/api/canvases/{id}/edges`, and **no MCP tool creates, lists or
implies an edge**; removing the edge, either panel, or the canvas removes the
...
[Canvas → Edges](canvas.md#edges-adr-0116).
