...
| `#/workspaces/<id>/overview` | Workspace overview | A page route opened by **Overview** in the workspace card's menu, never an editor tab. It shows workspace Inbox requests, Git/PR facts, live agents and terminals, active missions, and scoped recorded session activity. The four data reads fail independently; agent state comes from the existing fleet/feed. `GET /api/workspaces/{id}/stats` owns the period totals and series (ADR-0207); the global dashboard stays machine-wide. |
