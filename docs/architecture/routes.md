...
| `#/instructions/<workspaceId>` | Instructions tab | one tab per workspace (`i:<workspaceId>`): which instruction files each agent CLI reads for a session started there, and why it skips the others ([cli-instructions.md](cli-instructions.md)). Opened from the workspace's `…` menu. Read-only; a file name opens it in the editor. |
