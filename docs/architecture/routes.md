...
| `#/file/t/<id>/<path>` | File tab | text editor for a path under that terminal's cwd (Ctrl+click in xterm). `#/file/a/<id>/<path>` is the same for the Pi TUI dock; `#/file/w/<id>/<path>` reads through a workspace (ADR-0030). Preview \| Raw for svg, mermaid, md, html, png, pdf, audio, video, glb/gltf (`GET …/blob`; HTML through `POST /api/previews` → a sandboxed `GET /preview/<ticket>/<path>`, ADR-0136). |
