...
| `#/clis/<cli>/skills` | Skills (ADR-0196) | what the CLI loads as Agent Skills: every `SKILL.md` folder it reads in its own order, the copy that wins, the installer's lock that names it, the cost at start; `?workspaceId=` adds the workspace's folders, `?scope=workspace\|machine` filters. Read-only in slice 1 ([skills.md](skills.md)). |
