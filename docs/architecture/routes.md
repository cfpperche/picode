...
| `#/inbox[/<id>]` | Core Inbox (ADR-0208) | Questions, approvals, results and reminders. The desktop sidebar header has a direct Inbox button with its own blocking count or activity dot; the phone keeps its Inbox tab. Old `#/app/inbox[/<path>]` links replace to this route. The browser renders the shared primitives from `/api/inbox/view` and posts `/api/inbox/action`; Inbox is absent from `GET /api/apps`. |
