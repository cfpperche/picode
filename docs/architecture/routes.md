...
**One scope vocabulary for setup tabs (2026-09-24).** Packages, Connectors,
Skills, Memory, Settings, Keyboard and Models address their scope as
...
| Hash | Surface | Owns |
|---|---|---|
| `#/` | Agent workspace | tabs, chat, terminal. Replaced by `#/agent/<id>` when an agent is open. With no tab open (or pinned via the logo — which also brings the workspace route back when the click comes from Agent CLIs, Browser, Preferences or Devices): the session observability dashboard once any workspace/agent/terminal exists — when something is blocked on the reader, one attention line above the numbers (count of Inbox questions from the core badge + agent-CLI terminals whose hooks said `needs-you`, with one action to the Inbox or that terminal; ADR-0109's 2026-09-14 amendment declares the door), spend/activity/sessions tiles over the chosen range plus a **Fleet** tile that is live and machine-wide (managed agents beside agent-CLI terminals, with shell terminals counted apart; states `working` / `need you` / `idle` / `no signal`, four rows open their agent or terminal tab and `+N more` reveals the rest — ADR-0042's 2026-09-13 amendment), a bar chart of the range — one bar per day, or per hour for `Today`, where the series key states its own granularity (`2026-09-14T14` is 14:00) — and spend by model / workspace, tokens, tools, reliability, top sessions, **Agent outcomes** from `GET /api/agent-exits/summary` for the same range (ADR-0194) (ADR-0041, ADR-0042; `GET /api/sessions/stats`, fingerprint-cached, meters aggregated in parallel with a background warmup after boot, last good payload replayed from localStorage while revalidating, polled every 60 s while visible; the terminal list it counts comes from the app's fleet state, no extra fetch) — else the first-run blank slate. Not routed, derived from `noTabs && hasData`. |
| `#/workspaces/<id>/overview` | Workspace overview | A page route opened by **Overview** in the workspace card's menu, never an editor tab. It puts questions, blocked/review missions, waiting agents and failing PR checks first, then project, live agent, mission and scoped session facts. Recent changes join the bounded event summary with recent Git commits; the browser's last-visit marker is local and limited by seven-day event retention (ADR-0210). `GET /api/workspaces/{id}/stats` owns the period totals and series (ADR-0207); the global dashboard stays machine-wide. |
| `#/workspaces/<id>/agents` | Workspace agents | A page route linked from the overview's prioritized agent summary. It lists all workspace agents with search, live status age, assigned mission or current checklist step, and a contextual action. Unbound terminals are separate. The overview and this page reuse fleet, checklist, inbox and mission state; neither opens an editor tab. |
...
| `#/instructions/<workspaceId>` | Instructions page | a page over the tabs, like Agent CLIs (since 2026-09-24; it was an `i:<workspace>` tab before, and a saved one is dropped on load): which instruction files each agent CLI reads for a session started in that workspace, and why it skips the others ([cli-instructions.md](cli-instructions.md)). Opened from the workspace's `…` menu; the address names the workspace, PageFrame draws Back and the title, the context line names the folder. A workspace that is gone shows one line and a way back. |
...
| `#/clis/<cli>/settings` | Native CLI settings (Pi first) | Pane of the selected CLI. Global + Keys without context; `?workspaceId=<id>` adds the workspace layer without selecting an agent (the same query the Models pane already carries); `?agentId=<id>` adds the actual workspace and agent layers. A missing or unknown workspace id is an error, never another workspace. Composer `/settings` includes the agent; `/scoped-models` adds `focus=scoped-models`. The Pi-era addresses `#/settings`, `#/more/settings`, `#/clis/settings/<cli>` and `?tab=keys` were retired on 2026-09-24. |
...
| `#/clis` | Agent CLIs | CLI catalog, installation checks, launch defaults and activity-reporting switches. The strip is **CLIs \| Messages \| Settings**; `#/clis/settings` is the Settings tab, the terminal-wide tmux guard and browser hand-off switches. A CLI's page hosts **Launch**, **Terminals**, **Sessions**, **Providers**, **Settings**, **Packages** and **Connectors** (a run/setup split). The Pi-era addresses `#/packages*`, `#/clis/packages*`, `#/mcps`, `#/clis/connectors`, `#/sessions*`, `#/clis/sessions*`, `#/providers*` and `#/clis/providers*` were retired on 2026-09-25; `#/integrations` opens Webhooks. `#/clis/new/<cli>` and `#/clis/terminal/<id>` edit launches. Desktop sidebar header / command palette and mobile More open this surface; the desktop rail follows the same context rule as the CLI panes (ADR-0179): with an agent selected it opens at that agent's CLI (`#/clis/<cli>`), without one it keeps `#/clis`. The palette, the composer's agent pages and mobile More follow the same rule for every CLI pane command (Settings, Packages, Skills, Connectors, Providers): the context agent's CLI (an agent with no `cli` is Pi's, `agentIsPi`), and with no agent the catalog `#/clis` — never a Pi default (2026-09-25). The old `#/preferences/status` address redirects here. |
| `#/system` | Machine facts | host, network, deps, version (read-only) |
| `#/clis/<cli>/providers` | Native providers pane | Pi: catalog + signed-in state; Sign in; search; **custom provider definitions** (ADR-0129); **plan windows on each account row** from the usage cache, live / stale-with-age / a reason (ADR-0058); vendor identity (email, plan); credential source (vault or an env var); **Verify** via `pi auth check`; **Usage** dialog per vault account (ADR-0031); Pause beside Sign out; 7-day spend per provider; Sign out names the agents and automations that break. `#/clis/<cli>/providers/new` opens Add provider; `#/clis/<cli>/providers/custom[/<id>]` opens the Custom provider page (new / Edit). The Pi-era `#/clis/providers*`, `#/providers*` and mobile `#/more/providers*` aliases were retired on 2026-09-25. Non-Pi CLIs stay blocked (ADR-0103). |
| `#/clis/<cli>/connectors` | Pi MCP connectors | adapter manager on the selected CLI: list / add / toggle / remove / **Use from…**. |
| `#/integrations/webhooks` | Webhooks (ADR-0075) — `#/integrations*` opens it too | signed durable event delivery, tests, pause, removal and secret rotation. PiCode surface, not a CLI pane. |
| `#/clis/<cli>/packages` | Packages — one pane for Pi and the eight guest CLIs (ADR-0167, ADR-0176) | the CLI's own report decides what is drawn: Pi keeps machine / workspace (`pi install`) / this agent (`-e` on start) with **Configure** → `#/clis/<cli>/packages/config/<pkg>`; a guest reads its own scopes, plugin verbs and marketplace from the same pane, and a verb the CLI lacks reads as one line instead of a control. `pi` and `omp` are the two CLIs that declare an agent layer — PiCode stores those entries and the CLI's launch passes them on as `-e`. |
| `#/clis/<cli>/skills` | Skills (ADR-0196) | what the CLI loads as Agent Skills: every `SKILL.md` folder it reads in its own order, the copy that wins, the installer's lock that names it, the cost at start; `?workspaceId=` adds the workspace's folders, `?scope=workspace\|agent` filters. **Add skill** previews a source (files, spec problems, advisory scan) and installs into `.agents/skills` with the lock the `skills` CLI reads; rows in `.agents/skills` offer **Update** and **Remove**; a row's switch turns the skill off in the CLI's own settings (`POST /api/skills/toggle`; Pi and Antigravity have none); **Check for updates** compares locks with their sources; `?agentId=` adds the agent's chip for Pi, Omp and Claude Code, where **Add skill** caches a skill for that agent alone; the **Marketplace** tab searches the catalog (`GET /api/skills/catalog`, built-in and added sources, skills.sh when switched on) and opens the same preview from a card ([skills.md](skills.md)). |
...
| `#/inbox[/<id>]` | Core Inbox (ADR-0208) | Questions, approvals, results and reminders. The desktop sidebar header has a direct Inbox button with its own blocking count or activity dot; its page header uses Back to `#/`, like Agent CLIs. The phone keeps its Inbox tab. Old `#/app/inbox[/<path>]` links replace to this route. The browser renders the shared primitives from `/api/inbox/view` and posts `/api/inbox/action`; Inbox is absent from `GET /api/apps`. |
...
| `#/outcomes` | Outcomes (ADR-0194) | the catalog of removed agents: range/workspace/CLI/outcome filters, KPI tiles and bars reused from the dashboard, rows that open to the whole record, answer later, delete a record, export JSON lines, and the **Ask when removing agents** switch. Refetches on `agent_exit.*`. Desktop only (user menu ▸ Tools, command palette). |
...
layouts from one document controller (`web/browser/src/lib/fileDocument.js`, mirrored in `web/mobile`). Dirty
...
Success HTML is PiCode's (logo + return to the CLI's `#/clis/<cli>/connectors`, with the workspace and agent it was started from). Add or On on an OAuth server
...
Entry: the dedicated Inbox icon in the desktop sidebar header opens `#/inbox`; the last icon opens Agent CLIs (`#/clis`)
without changing the rail tab; user menu (Tools: Automations, Snippets,
llama.cpp; PiCode: Preferences, Browser, Computer, Devices, System,
Integrations, Termset) and `Ctrl+K`.
