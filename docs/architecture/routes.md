...
| `#/clis` | Agent CLIs | CLI catalog, installation checks, launch defaults and activity-reporting switches. A CLI's own page hosts four panes: **Launch** (`#/clis/<cli>`), **Terminals** (`#/clis/<cli>/terminals` — the one list of that CLI's terminals, under the shared row menu `termRowMenu.js`), **Sessions** (`#/clis/<cli>/sessions`, machine-wide grouped by folder, or `#/clis/<cli>/sessions/<workspaceId>` for one folder) and **Providers** (`#/clis/<cli>/providers`, Pi roster; other CLIs show a blocked notice). The general Terminals, Sessions and Providers tabs left the strip on 2026-09-11; `#/clis/terminals`, `#/clis/sessions[?cli=]`, `#/sessions*` and `#/clis/providers*` rewrite onto those panes. `#/clis/new/<cli>` and `#/clis/terminal/<id>` edit launches. Pi keeps full session management; other CLIs list and resume-in-terminal (ADR-0079). Desktop user menu / command palette and mobile More expose their own copies of this surface. The old `#/preferences/status` address redirects here. |
| `#/system` | Machine facts | host, network, deps, version (read-only) |
| `#/clis/<cli>/providers` | Native providers pane | Pi: catalog + signed-in state; Sign in; search; **plan windows on each account row** from the usage cache, live / stale-with-age / a reason (ADR-0058); vendor identity (email, plan); credential source (vault or an env var); **Verify** via `pi auth check`; **Usage** dialog per vault account (ADR-0031); Pause beside Sign out; 7-day spend per provider; Sign out names the agents and automations that break. `#/clis/<cli>/providers/new` opens Add provider. Old `#/clis/providers*` and `#/providers*` rewrite here. Non-Pi CLIs stay blocked (ADR-0103). |
