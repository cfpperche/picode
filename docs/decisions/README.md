...
| [0056](0056-guest-tui-state-sensors.md) | Guest CLI state in two tiers — tier 1 (built): terminals report via per-tool sensors on the change feed (`working` / `needs-you` / no signal); tier 2 (deferred): promote to observe-only guest agents. Screen-scraping refused; ACP/control deferred | accepted 2026-09-03, tier split fixed in review; builds on 0017, 0024, 0048 |
