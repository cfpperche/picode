...
| [0086](0086-process-cost.md) | The rite around a change costs less than the change: batched guarded deploys, scoped gates, `make close`, a 100-line handoff with per-session files, advisory capture parity | accepted, amends 0018 and 0084/0085; amended 2026-09-08 (docs site path `www/` renamed to `docs-site/`) |
...
| [0098](0098-windows-clean-install.md) | Windows clean-machine install — the desktop exe installs the Linux binary and the runtime (tmux, Node, pi) itself, gets signed, ships via winget; imported PiCode distro as a later opt-in | accepted, extends 0020 and 0093, amends 0003's first-run boundary |
...
| [0100](0100-pin-reminders.md) | Pin reminders — a `pin_reminders` row (once / interval with schedule-or-completion anchor / cron in a named zone) fired by the one-minute engine into an Inbox item that is the acknowledgement state; sticky notice + push as projections; missed slot fires once | proposed 2026-09-08, builds on 0037, 0045, 0048 |
