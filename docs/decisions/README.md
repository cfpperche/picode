...
| [0062](0062-terminal-cli-presence.md) | Authoritative ephemeral CLI presence in project terminals via wrapper leases, run IDs, process validation, and exact tmux/PID fallback; presence stays separate from lifecycle activity | accepted, amends 0056; amended by 0112 and 0212 |
...
| [0105](0105-process-cost-second-review.md) | The rite runs in a fresh context, the living docs stop conflicting, deploy is the owner's call: one branch one session, changelog fragments, handoff without shipped work (100 lines and 8 KB), no deploy timer, captures at deploy, sharded server tests, `make adr` with a boundary line, architecture split per subsystem | accepted, amends 0086; amended by 0149 (a correction to a handoff note that already landed, and a new topic file, are allowed on `main`; fragments and new notes still travel with the branch); amended 2026-09-23 — the macOS leg joins the per-push matrix for diffs touching the paths where macOS has broken before (`macosRelevant` in `scripts/ci-scope.mjs`), the defects that made it unread being fixed; amended 2026-09-24 — the two ~26-minute packages (`internal/server`, `internal/store`) run in a job of their own (`go-heavy`), on the same platform list, so a slow package cannot take the rest of the suite past a shared ceiling |
...
| [0160](0160-cli-runtimes-are-agents.md) | CLI runtimes are agents — catalog `cli` on `agents`; managed RPC stays Pi-only | accepted, supersedes 0159's never-an-agent-row, amends 0006/0011/0069; unbound CLI terminals ended by 0184; Fatia F working gate amended by 0206 |
...
| [0173](0173-sidebar-order.md) | Sidebar order is a stored position shared by every client; drag is a later gesture; presentation-only views (Working first) reorder rows without writing positions; inside the view, not-working buckets rank by park time (2026-09-24) | accepted |
...
| [0184](0184-one-cli-door.md) | One door for agent CLIs — every user-facing CLI launch is an agent; `POST /api/clis/{cli}/terminals` removed; sign-in terminals internal and reaped; Make agent for a CLI in a PiCode shell | accepted, amends 0160 and 0143; browser session-drive row amended by 0188 |
...
| [0188](0188-terminal-session-drive.md) | A terminal identity keeps the browser's session drive — a shell or sign-in with no agent drives its own split, nothing more | accepted, amends 0184 |
| [0189](0189-claude-code-platforms.md) | Claude Code on Bedrock, Foundry or Vertex from the GUI: PiCode writes the settings.json env block Claude Code's own /login wizard writes; a platform outranks and replaces the subscription or Console key, and choosing either removes it | accepted (owner, 2026-09-22), extends 0187 |
| [0190](0190-claude-code-gateway.md) | Claude Code through an Anthropic-compatible gateway (base URL + token in settings env, ADR-0189's mechanism); with a gateway in use no Console key is injected, since Claude Code would send it to the gateway too; a URL without a token is the person's proxy and is never touched | accepted (owner, 2026-09-22), extends 0189 |
| [0191](0191-codex-gui-login.md) | Codex signs in from the GUI through its own app-server (ChatGPT browser, device code, API key, Bedrock); Codex writes its files; a non-Bedrock login removes the model_provider = "amazon-bedrock" Codex leaves behind | accepted (owner direction, 2026-09-22) |
| [0192](0192-grok-gui-login.md) | Grok signs in from the GUI by running its own login (browser, device code) and showing its page and code; an xAI key is used only when chosen, which files and signs out Grok's session first; Use writes the session back from the kept copy | accepted (owner direction, 2026-09-23) |
| [0193](0193-hermes-gui-credentials.md) | Hermes adds credentials from the GUI through its own `hermes auth add` (key on stdin, OAuth as a device code), new ones last in its pool; its roster reads Hermes's own provider registry | accepted (owner, 2026-09-23) |
| [0194](0194-agent-exit-records.md) | Removing an agent writes an exit record — frozen setup, observed signals, an optional outcome — in the same transaction as the delete; turns are counted on the agent; the server decides when to ask; taxonomy v1; nothing leaves the machine | accepted (owner direction, 2026-09-23) |
| [0195](0195-muse-gui-login.md) | Muse signs in from the GUI by running its own `muse login` (device code); a Meta key is applied with Use, which first files the login Muse's one slot holds | accepted (owner direction, 2026-09-23) |
| [0196](0196-skills-for-every-agent-cli.md) | Skills for every agent CLI — one reader over each CLI's own skill folders; installs write those folders and Vercel-compatible locks (`skills-lock.json`, `~/.agents/.skill-lock.json`); native toggles; per-agent sets at launch; a federated catalog that never vouches, skills.sh opt-in; amends 0167 for skills | accepted (owner, 2026-09-23); launch facts corrected by slice 4 (2026-09-24) |
| [0197](0197-agy-gui-login.md) | Antigravity signs in from the GUI: PiCode runs its print-mode sign-in under a pseudo terminal, shows Google's page, passes the pasted code in and stops agy when the token lands (a present login set aside and put back on failure) | accepted (owner, 2026-09-23) |
| [0198](0198-management-root-in-distro.md) | management-root-in-distro | accepted |
| [0199](0199-mission-persistence-contract.md) | Persistent missions and versioned outcomes | accepted |
| [0200](0200-mission-execution-transfer.md) | Mission assignment and controlled transfer | accepted |
...
| [0206](0206-attach-delivery-modes.md) | Attach delivery modes — steer and follow-up into a working CLI through per-CLI adapters; `queued` receipt; needs-you, drafts and automations unchanged | accepted, amends 0089 and 0160; amended 2026-09-24 (Stop and send; Hermes `accepted` receipt, Omp Ctrl+Q and quiet stop) |
