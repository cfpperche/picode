...
- `GET /api/{agents|terminals|workspaces}/{id}/gitstatus` — the working-tree
  ...
  page carries `branch`, `worktree` (linked worktrees only) and `totals`
  (`add`, `del`, `files`) summed over the re-anchored slice.
...
Real-RPC proof passed against an unpatched 0.6.6 checkout; daemon feed
forwarding and the UI subscription are the remaining phases. The superseded
in-package patch and the panel are tracked in the
