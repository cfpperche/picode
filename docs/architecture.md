...
Click a path on an `edit`/`write` card opens that file beside the chat (`GET/PUT /api/agents/{id}/text`, cwd only). Save writes the file. A stale mtime is 409 (open again).
