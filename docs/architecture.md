...
server starts Sign in immediately. The GUI waits for the adapter's localhost
callback (same shape as provider OAuth); paste is a fallback. Tokens live in the OS keyring, keyed by server
