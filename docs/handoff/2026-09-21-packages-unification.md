...
Verified: `go test ./internal/pkgs/` and `go test ./internal/server/ -run TestPackageReport` green here; `git diff --stat main...HEAD -- web/` empty, which is the no-regression proof; gates: `make ci-scoped` running at note time (Go + docs scope) — no result claimed. Method blind spot: no live renderer or browser pass, because no pane changed.
visual-review: n/a
