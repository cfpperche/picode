...
Blind spot: the 30 s readiness fix was not measured live, and the real Tauri shell was not run end to end with this page.
...
Merge: `make close` pending at time of writing.

## Next up

- Owner runs `make deploy` then `make desktop-restart` (serialized), opens Management: no terminal, streamed scan, and the main window's first paint no longer ~30 s.
