...
- [ ] Cost per agent is not recorded (climetrics measures per CLI and session, ADR-0127); the exit keeps the session pointers so it can be derived while the files exist.
