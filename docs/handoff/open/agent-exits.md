...
- Options C and D wait on a picode MCP verb for structured proposals (the CLI-neutral unattended runner exists since ADR-0217: `start` runs on Claude Code, Codex, Grok and Hermes); D also needs a security-model ADR (a reviewer reads every agent's transcript and writes repository files).
...
- [x] Agent history has no phone screen. — paid 2026-09-24 (feat/mobile-history): More ▸ Agent history.
...
- [x] Measure real Agent History `GET /api/agent-history` latency on the owner's Windows dataset after deploy — paid 2026-09-24 by observation: after the 8979d63dc deploy production answered in 0.02 s and the owner confirmed the page opens fast; the synthetic fixture benchmark does not establish it. Note 2026-09-24 (feat/agent-history-v2): the locator was measured against the owner's real dataset on this machine (WSL, 13 exits, 0.24 s); Windows still unmeasured.
