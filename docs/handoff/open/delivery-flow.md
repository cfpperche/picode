...
- [x] Vendor CLI processes have now exercised the delivery tool end to end (2026-09-21, scratch only): **Claude Code** (headless with PiCode's `--mcp-config`, and driven through PiCode's prompt door), **Codex** (failed first — see the identity debt below — passes after the fix), **grok**, **omp** (both inherit the launch environment), **hermes** and **muse** (declared with the identity set explicitly on the server entry, which those two require). Each declaration landed with `principal` = the agent id and the project read shows it. Not exercised: **Antigravity** (no binary on this machine), and no CLI was run against a *live* model with the owner watching.
- [ ] Hermes and Muse hand their MCP servers an **allowlist** of environment variables (13 and 8 names; measured with an env probe — Codex hands none), so a PiCode tool server reaches them with no identity and answers `no identity`. A literal per-server `env` works for both (measured: 0 → 2 `PICODE_*`), but a connector written at user/workspace scope has no per-terminal value to write — the launch path would have to carry it, as it now does for Codex (a design decision, not a fix).
