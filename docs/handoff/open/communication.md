...
- Onboarding: initial Codex/Hermes/OpenCode and resumed Codex/Hermes required a first prompt in real matrix validation; fresh Claude waits for a saved first conversation (see `docs/plans/cli-attention-matrix.md`); rows 12 of `docs/plans/communication-onboarding.md` (child timeout) remain partial. Six native CLIs passed paired transport acceptance with correct providers and loaded adapters; see `docs/plans/communication-native-finish.md`.
