...
- **`TestPaneRootSurvivesSIGHUP`** (same commit as the peer-stop test) failed
  ...
  first owns this line. `feat/peer-stop-child-race` adds the second half of the
  SIGHUP story: the fixture names are process-unique (`…-qa-<pid>`), because
  two test binaries closing at once were also sharing the fixed
  `picode-sh-hup-*-qa` names and reading each other's panes.
