...
- Terminal menus (2026-09-09): web/mobile rows offer only Remove; the desktop one-menu merge (`termRowMenu.js`) is unported — desktop **Continue in…** (2026-09-13) waits on it for mobile.
...
- tmux: never kill by prefix (a `grep '^picode-'` sweep killed 29 sessions, six in production, 2026-09-06) — exact names from a fixture's API; a scratch whose daemon dies before `qa-scratch stop` strands its shells.
