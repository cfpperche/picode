...
- CLI launch fixtures: `waitCLIFile` only waits for the path to exist, so a second launch into the same file needs the file removed (or a wait on its own argv) — the resume row read the creation launch's bytes shard-shifted until it was fixed.
