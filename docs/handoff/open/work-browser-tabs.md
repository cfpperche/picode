...
- **Owner's calls on the browser's remaining items (2026-09-19, one session):**

  | Item | Call | Why |
  |---|---|---|
  | Import cookies and passwords… | **refused** | Passwords have no write API in WebView2 — that is PiCode owning a vault (DPAPI at rest, Hello unlock, CDP fill), a security-model arc of its own. Cookies alone would work through CDP, but Chrome's app-bound cookie encryption makes a reliable import fragile, and the work profile keeps the logins you make once. The ⋮ entry stays out and the parity list records the refusal instead of a forever-missing item. |
  | History access "Always ask" | **deferred** | The switch exists (`never`/`allow`); no agent has ever asked for history. A held tool call plus a prompt surface plus remembering the answer is a lot for demand that has not appeared. If it does, the cheap shape is a notice with one-click Allow, not a held request. |
  | Device toolbar | **native half first** | Resizing the view's bounds + `ZoomFactor` needs no ADR and covers width-based media queries; the honest name is "Responsive width". True device emulation (`Emulation.setDeviceMetricsOverride`: mobile UA, touch, devicePixelRatio) drives CDP, which today is the *agent* gate (ADR-0144) — that half opens an ADR when it is wanted. |
  | Search from the address bar | **not built** | Copy made honest instead ("Enter a URL"). |
...
- **Import cookies and passwords…** — **refused by the owner (2026-09-19)**
  ...
  Chrome-decryption path for little gain.
