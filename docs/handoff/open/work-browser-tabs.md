...
- No JS check catches use-before-declaration in a component (the blank-window
  class).
- **v2d's row has never been clicked on Windows**: the pure allowlist tests
  (`rustc --edition 2021 --test src/external.rs`), the JS payload test and the
  cross-build are the evidence here; the OS screen itself is the owner's first
  run, like every Windows-only path in this file.
