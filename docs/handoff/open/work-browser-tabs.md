...
landed 2026-09-14 and was **accepted live on Windows** the same day: the
snapshot (962 KB AX tree) and the screenshot (PNG, 1053×1400) round-tripped
through the bound pane. Closing rite of that branch still pending (changelog
fragment + `make close`).

## Next

- **Owner directive (2026-09-14): DONE on this branch** — the split/pane state
  survives a desktop relaunch: layout + last url persist in localStorage
  (`picode-agent-splits` / `picode-agent-split-urls`), boot prunes panes whose
  host tab is gone, and the webview is recreated at the last url the first
  time the host tab is shown. One post-deploy check remains in the real shell
  (that recreate line is shell-only).
...
- The CDP bridge and the channel got their Windows runtime acceptance on 2026-09-14: snapshot + screenshot round-tripped through the bound pane.
- `make desktop-restart` swaps the Go tray + native host only; the v2 shell
  (`picode-shell.exe`) must be rebuilt (`make desktop-shell`) and copied into
  `%LOCALAPPDATA%\PiCode` by hand. A stale shell refuses `btab_cdp_call` with
  a Tauri ACL error, because the capabilities compile into the exe (paid for
  2026-09-14).
