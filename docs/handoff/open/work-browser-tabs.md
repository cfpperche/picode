...
- **Owner directive (2026-09-14): DONE on this branch** — split layout + last
  url persist in localStorage; boot prunes dead hosts; the webview is
  recreated at the last url when the host tab is shown. Post-deploy: re-check
  that recreate line in the real shell.
...
- CDP bridge/channel Windows runtime acceptance: done 2026-09-14 (snapshot +
  screenshot through the bound pane).
- `make desktop-restart` swaps the tray + native host only; the v2 shell needs
  `make desktop-shell` + a manual copy — a stale exe refuses `btab_cdp_call`
  (Tauri capabilities compile into it). Paid for 2026-09-14.
