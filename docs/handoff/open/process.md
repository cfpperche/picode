...
- Hook edits cannot be exercised from the making worktree: every worktree runs the *root* checkout's hooks; `make hooks-check` on a throwaway repo is the local proof; a commit the branch's own hook allows needs `git -c core.hooksPath=$PWD/.githooks commit`.
