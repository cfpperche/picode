...
- [x] Pi settings deep link (`#/clis/<cli>/settings?workspaceId=…`) drops
  ...
  should carry the workspace like the models pane does. Paid in
  feat/mission-settings-deeplink: the parser, hash, Settings/Keyboard tabs and
  `/api/pi-settings` keep the workspace; a missing id is an error, not another
  workspace or an inferred agent.
