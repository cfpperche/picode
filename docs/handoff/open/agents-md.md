...
- [ ] Antigravity 1.2.10: when (if ever) `GEMINI.md`/`AGENTS.md` reach the model — measuring it needs a run that may use a tool, i.e. an allow-rule in `~/.gemini/config` or an interactive run by the owner.
