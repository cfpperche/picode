...
- [ ] `disk-compact` stops the distro while the `PiCodeDistro` task restarts on failure within a minute; it now holds the distro (wsl-mgmt-history), but should also disable the task like move/backup do.
