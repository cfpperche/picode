...
- [ ] Owner, production certificate (2026-09-24): `~/.picode/cert.pem` covers localhost, picode.local, the Tailscale name and two LAN IPs but not 127.0.0.1 / ::1. `picode provision` now reissues it from the same mkcert CA with every current name kept plus loopback (`TestCertStepReissuesACertificateWithoutLoopback`); not run here because it rewrites the live certificate. Check after: `openssl x509 -in ~/.picode/cert.pem -noout -ext subjectAltName`.
