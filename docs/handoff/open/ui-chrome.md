...
- Fetches (2026-09-13): with a file, tree, git or app tab selected the shell still asks `/api/agents/<tab id>/role-state` and `/slash` — only terminal tabs are excluded (`App.jsx`), so each selection costs two caught 404s. Pre-existing.
