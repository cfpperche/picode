...
- Pilots 1–4/5 completed: `mission_WUFG3EEGTSC3LAAM54DRTWPGJF` used dedicated Omp/manual context paste and a stale-main review; `mission_N7K4X3IA26T6AQC66EEW27LWSO` used Codex → Claude Code transfer and a copy correction after review; `mission_6S3Q5CSTFZIZ4R2MNPTSQSJZCE` paused and resumed the same native Codex session before fixing missing MCP mutation fields; `mission_F73FEWS6TJ277I7S3FYC53XN5W` used managed Pi for a Settings deep link, with owner-attributed evidence because native Mission reporting was absent in the deployed PiCode. All four were accepted, landed and passed full CI. Run one more real mission, including a physical-phone answer; compare observed context-recovery time with five baseline tasks before claiming a gain. Accepted records and preserved pilot 4 screenshots are under `var/qa/missions-pilot/`.
...
- Native Pi Mission registration and attributed writes were exercised in isolated Pi 0.87.1 managed RPC and PiCode interactive TUI scratch sessions. Both acknowledged and reported with PiCode-derived session bindings; see `docs/handoff/2026-09-24-mission-pi-native.md`. Broader live provider validation beyond `xai/grok-4.6` remains open.

## Debts

- [ ] Accepted `kind: file` evidence is only a relative path and digest, not a retained artifact. In pilot 1, six screenshots lived under `.worktrees/mission-dev-flow-outline/var/screenshots/`; after normal cleanup, the completed history remained but the original paths no longer resolved. Completed note-only pilots 2 and 3 also returned observation `available: false` / `the working folder is unavailable` after cleanup. Verified copies of pilot 1 screenshots and all three accepted records are under `var/qa/missions-pilot/`. In pilot 5 the owner could read report text but could not open the cited screenshots or logs from desktop or mobile. The review UI now labels those paths as references. Decide a durable evidence policy (snapshot bytes or a managed artifact location) and show completed history without an alarming missing-worktree observation.
...
- [ ] Measure live provider behavior beyond Codex 0.156.1, Claude Code 2.1.280 and the Pi 0.87.1 `xai/grok-4.6` managed/interactive scratch runs; one provider does not establish broad provider acceptance.
