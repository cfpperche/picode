...
Browser-capture sidecar (ADR-0082): `capture-sidecar-mobile-live.png` shows the
...
### llama.cpp jobs (delivery 2)

`llama-*-activity-*.png` and `llama-jobs-qa.json` cover empty activity,
per-file progress, pending cancellation, unknown outcomes, refresh errors and
unverified cancellation in light mode on desktop/mobile. Generated by
`scripts/qa-llama-jobs.mjs` against a disposable fixture with synthetic jobs.
`llama-live-*.png` and `llama-jobs-live-ui.json` use the real b10809 CPU router
and Qwen3-4B: load across navigation/reload, unload and shared mobile history.
`scripts/qa-llama-jobs-live.mjs` refuses a non-fixture PiCode server.
