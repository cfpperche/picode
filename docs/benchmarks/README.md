...
| [Exit feedback and agent-learning loops](2026-09-23-agent-exit-feedback.md) | Who asks, catalogs and learns when an agent's work ends: Claude Code `/insights` and session survey, Devin Session Insights, CodeRabbit, Copilot Memory, Greptile, Warp, Cursor/Augment/Windsurf memories, Factory Agent Readiness, Zed; ACE, ReasoningBank, CLI-agent failure taxonomies | The exit record at removal (ADR-0194): outcome apart from reason, one taxonomy, provenance, approval before behaviour changes, measure the effect | Live vendor docs, papers and third-party reverse engineering, 2026-09-23 |
