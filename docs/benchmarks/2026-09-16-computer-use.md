...
- **Sources:** fetched live on 2026-09-16 by a research sweep (eight
  ...
  OmniParser, and (added 2026-09-17) lahfir/agent-desktop — README, `CONCEPTS.md`,
  `docs/phases.md`, PR #110; crates.io metadata for xcap, windows-capture, enigo, rdev,
...
| **agent-desktop** (lahfir; read 2026-09-17) | Apache-2.0 · 1.2 k★ · 81 forks · created 2026-02-19 · v0.9.2 2026-09-17 · Rust CLI + C-ABI library (Python/Swift/Go/Ruby/Node/C) | **OS accessibility tree only** — `snapshot`/`screenshot` per app, "progressive skeleton traversal" (78–96 % fewer tokens: Slack 30,743 → 383), drill-down from a ref; `launch --cdp` opens a CDP endpoint on Chromium apps | Snapshot-qualified refs (`@s8f3k2p9:e3`) re-identified at action time — a changed target returns `STALE_REF` instead of a wrong click; **headless** default = accessibility-only, no focus steal, no cursor; `--headed` opt-in for physical input; machine-wide interaction lease; JSONL session trace with pre/post screenshots | none (macOS needs Accessibility + Screen Recording + Automation permissions) | **macOS only today**; Windows is Phase 2 on `feat/windows-adapter` (opener PR #110 still open 2026-09-15; `crates/windows` on main is a stub) with the same crates we listed (`uiautomation` 0.25, `windows-capture` 2.0) and the Chromium 138 UIA fact; Linux Phase 3, MCP server Phase 4 — all planned. The closest match to our proposal shape: refs + fail-closed re-identification + headless/headed split + trace. Watch item and a candidate to embed in the shell once Windows lands; a pattern reference until then |
