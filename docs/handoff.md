...
- `feat/matrix-store` — Matrix phase 2 (ADR-0108): migration 041 (040 is `feat/unified-messages`'; either order lands), store + six `matrix.*` events, `/api/matrices`, `web/shared/domain/matrix.js`; fragment `docs/changelog.d/matrix-store.md`; ff-ready.
...
2. Matrix app (plan `docs/plans/matrix-app.md`, phase 0 study `docs/benchmarks/2026-09-09-matrix-live-grid.md`): phase 1 `feat/apps-native-surface` in flight, phase 2 `feat/matrix-store` ff-ready; phase 3 `feat/matrix-surface` (grid, chunk loading, picker, `nextSlot`/`layoutDiff` in `matrix.js`) after both merge.
