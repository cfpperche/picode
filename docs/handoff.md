...
8. Git graph write actions (ADR-0096): all four phases shipped and reviewed adversarially (amendment in the ADR) — thirty-odd actions in risk tiers through ADR-0078's three doors, server-side composer (`internal/gitcmd`), typed confirmation for tier C by the run door, worktree+agent in one gesture, undo where an honest inverse exists. Inspector debts left: `git ls-files` search, per-anchor watch, `+N −M` footer.
