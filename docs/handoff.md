...
- Providers: API key + OAuth. Claude/Codex loopback `53692`/`1455`. Copilot/Kimi/xAI **device-code**. Radius stays TUI (gateway URL). **Usage** (ADR-0030) on `#/providers` for those OAuth plans — live 5h/7d/week/extra; API keys have no button.
...
- **2026-08-31** — **Provider Usage dialog** (ADR-0030) on `feat/provider-usage`.
  `#/providers` shows **Usage** only when `quotaKind` matches the active
  oauth slot (Claude, Codex, Copilot, Kimi, xAI). `GET /api/providers/{id}/usage`
  fetches vendor windows in-process; tokens stay on the server. Dialog:
  skeleton → bars / empty / error / Sign in. Statusbar still does not
  invent quotas. visual-review: PASS (usage-windows/empty/error/loading/auth,
  overlayAudit ok).
