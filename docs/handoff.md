...
- `feat/canvas-chrome` — the Canvas surface loses `.ft-head`: the plane fills the stage and the chrome floats on it as `.cv-cluster` (switcher + Add panel + a `⋯` menu carrying New canvas / Tidy / Rename / Delete / Close tab top-left; zoom + minimap bottom-right). `Fit` reserves the corners; a maximized panel hides both clusters. **Session two takes this same branch** for the background-pattern preference and the light-palette rework: the clusters are the one place to change a ground (`.cv-cluster` in `web/desktop/src/styles/canvas.css`), and `--bg-elevated` is still an alias of `--bg-panel` — splitting them is that session's call. `ci-scoped` green, unmerged, not deployed.
...
- Canvas v2 (C2–C4, 2026-09-10): `+`/`-`/`0` need a focused panel; a still stamps its age only when the feed moved; only the picker adds a `file`/`diff` panel (ADR-0109). Chrome (2026-09-11): floating clusters overlap content by design — `Fit` clears them, a dragged panel can still hide under one. Edges (ADR-0116): no keyboard path to *draw* a link, so the Messages audit list is the keyboard way to read and revoke, at one canvas read per canvas (capped at 50).
