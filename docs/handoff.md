...
- Matrix (ADR-0108/0109): no docs-shots capture — needs a `desktop-matrix` profile plus a fixture that seeds one. Attaches are bounded by scroll *speed*, not distance (the 300 ms dwell). Seen in its captures, app-wide: the toast covers a surface's Close; dialogs have no scrim and little dark elevation; after a grow-resize an idle pane's cursor waits for the next redraw.
