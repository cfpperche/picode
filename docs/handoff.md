...
- MCP **Sign in** is automatic like providers: short `pi -e` runs headless `authenticate()` (no paste). Pi does not open the browser (WSL PowerShell would). Status returns the authorize URL; the GUI `window.open`s it once so the callback can close. Success page is PiCode's (logo + return to `#/mcps`). Overlay ends on the result file.
