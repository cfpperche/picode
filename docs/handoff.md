...
0. Desktop v2 (ADR-0120): dead frame buttons root-caused to the ACL (remote origin trust granted); plan agreed: frame mode in the served UI + dataset.frame handshake retires the injected fallback; ADR queued. Next: Phase 3 CDP browser - docs/plans/desktop-v2.md.
