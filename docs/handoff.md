...
- `feat/canvas-only` — ADR-0118, both sessions, **complete**: migration 045 converts grid panels once and renames the three tables; `/api/canvases…`, `canvas.*`; the desktop surface is `components/canvas/` (`CanvasSurface`, `Plane`, `Link`, `CanvasLinks`, `.cv-` classes), the word is gone from the app and `docs-site/guide/canvas.md`, `#/app/matrix[/<id>]` and `x:matrix` redirect, the two per-viewer keys migrate, react-grid-layout/react-resizable are out and the surface is lazy (−18 KB gzip first load). `ci-scoped` green, unmerged.
