...
- Also on `main`: File Tree v2 (0074), Git Graph per worktree (0073),
  ...
  ADR-0089 attach bar `0.1.0+aca6628`. GitHub repo picker in the clone form (`0.1.0+a2e699c`, accepted live).
  Workspace card: own folder/branch pills (the git graph of a project with no agents — ADR-0027/0030 routes desktop never called) and two actions instead of five, deployed `0.1.0+0dd28c0`.
...
- Handoff (ADR-0088/0094): visual pass done 2026-09-07, live-source warning never rendered (a Go test covers it); upstream formats undocumented (bump = refused write); Codex/Grok list a handed-off session only after a restart or by id; Hermes' importer flattens tool calls.
