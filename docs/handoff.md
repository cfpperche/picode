...
2. Matrix phase 4 (`docs/plans/matrix-app.md` §5): desktop `useAgentSocket`, read-only conversation body for managed agents, Needs-you chip; then the v1.1 list.
...
- Process (ADR-0105): worktrees start with a cold Go test cache (keyed by directory); `.pi/compact.json` `atPercent 0.5` never fires for large-window models (peaks 379 K) — owner declined config changes 2026-09-09; capture tolerance is a 128 px budget (`scripts/docs-shots.mjs` prints it per surface).
- Communication (ADR-0104/0106/0107): Claude/OpenCode full model exchanges await native account capacity (owner); physical-mobile and non-Linux pane/process recovery, and custom Codex resume global args/`--`, unverified. PTY rechecks cannot remove the check-to-write race; uncertain attempts never auto-retry. Never share credentials across conversations. Orphan private setup files need cleanup after owner deletion; revoked ones cannot authenticate. OpenCode inline merging needs JSON.
- Communication onboarding (ADR-0110): Codex SessionStart before a first turn unverified; the Claude/OpenCode/Grok/Hermes matrix not rerun. Partial rows 5, 9, 12 in `docs/plans/communication-onboarding.md`: moved-owner consent, missing-adapter repair, native stubborn-child timeout.
- Native packages/providers: real downloads, vendor OAuth, credential changes and device acceptance remain external; mobile package configuration is desktop-only. Tables in `docs/plans/cli-native-packages.md` and `cli-native-providers.md`.
- Native settings: physical iPhone/PWA/IME and a real process restart remain external acceptance; scratch tests cover recovery, retained drafts and stopped-agent saves.
...
- Handoff (ADR-0088/0094): upstream formats undocumented (bump = refused write); Codex/Grok list a handed-off session only after a restart or by id; Hermes flattens tool calls on import.
...
- `internal/server` tests swap package-level probes, so `t.Parallel` would race; `scripts/go-test.sh` shards them by process.
- `.git` is ~530 MB: UI bundles committed 339 times, tutorial MP4s re-rendered; a history rewrite is the owner's call.
...
- 2026-09-06 incident: a `tmux ls | grep '^picode-'` sweep killed 29 sessions, six in production. Never kill by prefix — only exact names from a fixture's API.
- Orphan tmux shells (2026-09-09): the `t.Context()` cleanups and `qa-scratch stop` are fixed; a scratch whose daemon dies before `stop` still strands its shells; `picode-sh-shell-6d4d44` is a leftover kept on purpose.
- Feed: ephemeral events can be missed across reconnects (ADR-0048); cross-platform paste fallback acceptance open.
- Terminal menus (2026-09-09): web/mobile terminal rows still offer only Remove — the desktop one-menu merge (termRowMenu.js) is not ported; sidebar Remove keeps `DELETE /api/terminals/<id>`, Agent CLIs uses `/launch/remove` (same outcome, two paths).
...
- Inspector: the Files filter covers loaded rows only; This-agent chips need the agent's tab selected; `gh pr view` answers are cached a minute.
...
- Mobile v2 (ADR-0095): physical IME/PWA/push/resume and microphone acceptance open; file writes keep the lexical/symlink and non-atomic mtime limits; iOS standalone strip needs on-device confirmation (Preferences → Layout has the dials).
- Matrix (ADR-0108/0109): no docs-shots capture — needs a `desktop-matrix` profile and a fixture seeding one. Attaches are bounded by scroll *speed*, not distance (300 ms dwell). App-wide, from its captures: the toast covers a surface's Close; dialogs have no scrim and little dark elevation; a grow-resize leaves an idle cursor waiting for the next redraw.
...
- Native surfaces (ADR-0109): `host` has no `openTerminal` — the Matrix POSTs `/api/terminals/{id}/open` itself (feed rows carry no `session`); a tab closing under a panel remounts the body with a fresh xterm. An unsupported tile explains itself only through `title`.
