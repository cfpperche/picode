...
- MCP **Sign in** is automatic like Claude/Codex: PiCode opens one tab, approve, overlay ends when the callback lands (token save continues in the background). A signed-in server says **Signed in** instead of Sign in. No paste.
