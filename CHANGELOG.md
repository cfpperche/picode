...
- MCP, Packages, and Settings name the selected agent as the first line inside the card (not under the title). Scope pills use the agent/folder name, not “This agent”. Clicking an agent in the sidebar from a pane opens that agent (`#/`) instead of silently retargeting the form.
...
- MCP **Use from…** mirrors other apps’ configs (you pick which). It does not copy files. Turn a mirrored server Off to hide it. Apps with no servers are not offered.
