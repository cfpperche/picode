...
- MCP **Sign in** is automatic like Claude/Codex: PiCode opens one tab, approve, overlay ends when the callback lands (token save continues in the background). A signed-in server says **Signed in** instead of Sign in. No paste. Add still saves the server if that login cannot run (GitHub Copilot has no dynamic client registration). Sign in forces a localhost callback so Linear does not dump you on linear.app.
