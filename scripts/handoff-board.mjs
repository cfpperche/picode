...
export function parseTopicFile(markdown, stem) {
  ...
  return { stem, title, next: pick(sections, NEXT_HEADINGS), debts: pick(sections, DEBT_HEADINGS) };
}
...
export function render({ worktrees, topics, notes, now = Date.now(), idleHours = 24 }) {
  ...
  const debts = [
    ...topics.flatMap((t) => attribute(t.debts, t.stem)),
  ...
  const lines = [
    ...
    "> *In flight* is git state; *next up* and *debts* come from `docs/handoff/open/<topic>.md`",
    "> and from the `## Next up` / `## Debts` sections of `docs/handoff/<date>-<branch>.md`.",
