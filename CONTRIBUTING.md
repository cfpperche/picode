...
Humans and coding agents contribute under the same contract. The full operating
...
1. Read `AGENTS.md` (auto-loaded) and `docs/handoff.md` (`make handoff`
   renders it; in flight, next up, debts) **before** starting; then only what
   AGENTS.md's reading table names.
...
| A slash command users can type | `docs-site/commands.md` heading `{#id}` + vendor correlation per [docs/guidelines.md](docs/guidelines.md) |
| Project state at all | `docs/handoff/<date>-<branch>.md` (≤ 25 lines) for the session, `docs/handoff/open/<topic>.md` for a durable next-up or debt — `docs/handoff.md` is generated by `make handoff` and is never edited (ADR-0123). A fact learned *after* the merge (a field confirmation, a debt paid by observation) is one commit on `main` amending the file it corrects — no branch (ADR-0149) |
