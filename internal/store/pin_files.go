...
func (s *Store) ListPinFiles(pinID string) ([]PinFile, error) {
	rows, err := s.db.Query(`SELECT id, pin_id, kind, name, mime, size, created_at FROM pin_files WHERE pin_id = ? ORDER BY created_at`, pinID)
	if err != nil {
	...
	for rows.Next() {
		var f PinFile
		if err := rows.Scan(&f.ID, &f.PinID, &f.Kind, &f.Name, &f.Mime, &f.Size, &f.CreatedAt); err != nil {
			return nil, err
		}
		out = append(out, f)
	}
...
func (s *Store) GetPinFile(pinID, id string) (PinFile, error) {
	var f PinFile
	err := s.db.QueryRow(`SELECT id, pin_id, kind, name, mime, size, created_at FROM pin_files WHERE id = ? AND pin_id = ?`, id, pinID).
		Scan(&f.ID, &f.PinID, &f.Kind, &f.Name, &f.Mime, &f.Size, &f.CreatedAt)
...
func (s *Store) AddPinFile(pinID, name, mime string, size int64) (PinFile, error) {
	...
	if _, err := s.db.Exec(`INSERT INTO pin_files (id, pin_id, kind, name, mime, size, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)`,
