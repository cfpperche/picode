...
func TestRunDecisionTable(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			dryRun: true, user: "goat", wantAction: ActionSkipped, wantFixes: 0,
		},
		{
			name: "root step without root is skipped", state: needsFix("needs it"), scope: ScopeRoot,
			isRoot: false, user: "goat", wantAction: ActionSkipped, wantFixes: 0,
		},
		{
			name: "root step with root is fixed", state: needsFix("needs it"), scope: ScopeRoot,
			isRoot: true, user: "goat", wantAction: ActionFixed, wantFixes: 1,
		},
		{
			...
			isRoot: true, user: "goat", wantAction: ActionSkipped, wantFixes: 0,
		},
		{
			name: "user step as root for root itself is fixed", state: needsFix("needs it"), scope: ScopeUser,
			isRoot: true, user: "root", wantAction: ActionFixed, wantFixes: 1,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			env := Env{User: tt.user, IsRoot: tt.isRoot}
...
func TestRunReportsAFixThatDidNotTake(t *testing.T) {
	...
	got := Run(Env{User: "goat"}, []Step{step}, false)
...
func TestRunDoesNotStopAtTheFirstProblem(t *testing.T) {
	...
	got := Run(Env{User: "goat"}, []Step{bad, good}, false)
	if len(got) != 2 {
