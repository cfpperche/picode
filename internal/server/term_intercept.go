...
// ompChecklistExtensionFile is the checklist mirror (ADR-0055): omp's native
// todo tool is the plan, and this extension mirrors its committed snapshot to
// PiCode's checklist routes so the sidebar shows the current step — the same
// projection pi-checklist gives pi and the picode-mcp family gives Claude
// Code. No tool is registered and nothing is gated: the todo contract, omp's
// own reminder and the TUI stay authoritative.
func ompChecklistExtensionFile(dataDir string) string {
	return filepath.Join(interceptDir(dataDir), "omp-checklist.ts")
}
...
// The checklist mirror observes two events and POSTs omp's own todo state to
// PiCode. tool_result{todo} carries the tool's committed snapshot
// (details.phases, the TodoToolDetails shape); session_start reconstructs the
// last snapshot from the branch — or resets the row, so a resume never
// resurrects a dead task's plan. Statuses map omp's five to PiCode's three:
// abandoned leaves the plan (the TUI keeps the history), blocked stays
// visible as pending work with its blocker named. Both handlers self-guard on
// the TUI: a headless -p run on the same terminal must not touch the row.
const ompChecklistExtensionTmpl = `import { readFileSync } from "node:fs";
...
func writeOmpIntercept(dataDir, hook string) error {
	...
	checklist := ompChecklistExtensionFile(dataDir)
	if err := writeInterceptFile(checklist, []byte(ompChecklistExtensionTmpl), 0o600); err != nil {
		return err
	}
...
func uninstallIntercept(dataDir, cliID string) error {
	...
	case "omp":
		...
		_ = os.Remove(ompChecklistExtensionFile(dataDir))
	case "agy":
