...
// Decision table, Fatia 3a/5/6: every integrable CLI carries a mechanism
// (arg branches, files, or environment). Muse Code joined in Fatia 6 when
// R3233 grew a hook surface (its 1.3.0-R3057 build had none); Antigravity
// reports through its title command since Fatia 5.
func TestIntegrationMechanismTable(t *testing.T) {
	...
	if p := cliIntegrationPlan("muse", t.TempDir(), "hook"); len(p.Files) != 1 || p.Summary == "" {
		t.Errorf("muse plan = %+v, want a summary and the hook file", p)
	}
}

// Decision table: the Activity toggle saves where a mechanism exists.
// Muse observes through its settings hooks since Fatia 6, Antigravity
// through its title command since Fatia 5; a CLI with hooks saves.
func TestIntegrationToggleGuard(t *testing.T) {
	...
	// On means installed: the row reports applied and the user's Muse
	// settings carry our observer hooks; off removes them again.
