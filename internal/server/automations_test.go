...
func TestDecideFire(t *testing.T) {
	cases := []struct {
		...
		{"message_to_a_guest_agent_ignores_missing_pi", fireInput{Enabled: true, PiMissing: true, Action: store.AutomationMessage, TargetExists: true, TargetIsPi: false, AgentMode: modeStopped}, fireDecision{}},
		{"message_to_a_guest_agent_in_its_terminal_is_skipped", fireInput{Enabled: true, PiMissing: true, Action: store.AutomationMessage, TargetExists: true, TargetIsPi: false, AgentMode: modeInteractive}, fireDecision{Status: store.RunSkipped, Reason: reasonInTerminal}},
	}
	for _, c := range cases {
