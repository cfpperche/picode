...
func UnitFile(execPath, pathEnv, home string) string {
	...
	// user's terminals and agent TUIs must survive daemon restarts. The
	// daemon itself must exit on SIGTERM below TimeoutStopSec; this
	// KillMode is the safety net so a stuck stop cannot SIGKILL the
	// tmux server (and every session) with the default control-group.
