...
export default function piDelivery(pi: ExtensionAPI) {
	...
	const tool = defineTool({
		...
		parameters: Type.Object({
			...
			revision: Type.Optional(Type.String({ description: "Full commit id that branch points at (register, update)" })),
			target: Type.Optional(Type.String({ description: "Local target branch, usually main (register, update)" })),
			id: Type.Optional(Type.String({ description: "Delivery id from a previous result (update, request-review, withdraw-review, show)" })),
			expectedVersion: Type.Optional(Type.Integer({ minimum: 1, description: "Version from the last result (update, request-review, withdraw-review)" })),
