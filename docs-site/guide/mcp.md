...
2. Agent CLIs → Pi → **Connectors** → Add a server (command or URL). **More** adds environment variables, headers, or sign-in (OAuth / token).
3. **Use from…** is the first chip under Add. It opens a tree of apps and their servers. Check the ones to use. It does not copy files. Shared servers show SHARED in the list.
