...
- **Where:** **Agent CLIs → Pi → Connectors**, then the **DeepWiki** card — or install `packages/pi-connector-deepwiki` through [Packages](/guide/packages).
...
2. Pick **one** path: the **DeepWiki** card in the Marketplace, or **Packages** → absolute path to `packages/pi-connector-deepwiki`.
3. Restart the agent.
