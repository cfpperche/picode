...
test("Shift+Enter is the default newline", () => {
  assert.equal(newlineSeq({ type: "keydown", key: "Enter", shiftKey: true }, "shift-enter"), "\x1b[13;2u");
  assert.equal(newlineSeq({ type: "keydown", key: "Enter" }, "shift-enter"), null);
});

test("Ctrl+Enter and Alt+Enter follow the pref", () => {
  assert.equal(newlineSeq({ type: "keydown", key: "Enter", ctrlKey: true }, "ctrl-enter"), "\x1b[13;5u");
  assert.equal(newlineSeq({ type: "keydown", key: "Enter", altKey: true }, "alt-enter"), "\x1b[13;3u");
...
test("Ctrl+C copies only when copyIfSelection is on", () => {
  ...
  assert.equal(copyPasteAction(ev, {}), "copy-if-sel");
});
