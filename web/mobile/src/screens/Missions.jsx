...
export default function Missions({ onBack }) {
  ...
  return <MissionPage key={loc.id + ":" + !!loc.create} loc={loc} onBack={onBack} />;
}

function MissionPage({ loc, onBack }) {
  ...
  return <div className="m-screen missions m-missions"><ScreenHeader title={v?.title || (loc.create ? "New mission" : "Missions")} onBack={onBack} /><div className="m-missions-body">
    ...
    {v ? <div className="mission-detail">
      ...
      <section className="mission-section"><h3>Acceptance criteria</h3><ul className="mission-criteria">{v.criteria.map(c => { const e = latestMissionEvidence(v, c.id, v.state === "completed" ? v.reviewRevision : data.observation?.revision); return <li key={c.id}><strong>{c.text}</strong><div className="mission-evidence">{e ? <><span>{e.outcome === "pass" ? "Reported passing" : "Reported failing"} · {e.actor === "owner" ? "Owner" : "Agent"}</span><p>{e.value}</p>{e.revision ? <code>Revision {e.revision.slice(0, 12)}</code> : null}</> : "No current evidence. Add evidence before review."}</div></li>; })}</ul>
...
function MissionEditor({ vm, base, workspace, onCancel, onSaved }) {
  ...
  return <form className="mission-form mission-section" noValidate onSubmit={submit}>
    ...
    <label>Title<input {...field("title")} placeholder="Add password recovery" /></label>
