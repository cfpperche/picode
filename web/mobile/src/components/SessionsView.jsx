import "../styles/mobile-sessions.css";
  ...
  return (
    <li className={"mcp-row sess-row" + (s.inUseBy ? "" : " orphan")}>
      ...
      <div className="mcp-row-actions">
        ...
        {s.inUseBy ? (
          <button type="button" className="btn btn-ghost btn-sm" disabled={busy} onClick={() => onCompact(s.inUseBy.agentId)} title={"Compact " + s.inUseBy.agentName + " (summarizes older turns)"}>Compact</button>
        ) : null}
        <button
          type="button"
...
export default function SessionsView({ wsId, workspace, agents, workspaces, onOpenAgent, onCompactAgent, embedded = false, cli = "pi", cliNames = {}, clis = [], wsReady = true, onNewTerminal }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const result = await api("/api/clis/" + encodeURIComponent(cli) + "/sessions" + query);
      if (generation === request.current) setData(result);
    } catch (e) {
    ...
    <PageFrame id="sessions-view" title={(workspace ? workspace.name + " · " : "") + "Sessions"} wide embedded={embedded}>
      ...
      {!data ? (
        ...
        <div className="cli-notice" role="status">
          ...
          <button type="button" className="btn btn-ghost btn-sm" onClick={() => query ? setQuery("") : (onNewTerminal ? onNewTerminal() : location.assign("#/clis/new/" + encodeURIComponent(cli)))}>{query ? "Clear search" : "New agent"}</button>
        </div>
      ) : (
        groups.map((g) => (
