...
const labels = { queued: "Waiting", running: "In progress", unknown: "Result unknown", succeeded: "Completed", failed: "Failed", canceled: "Canceled", interrupted: "Interrupted" };
  ...
  return <section aria-label="Model activity">
    ...
    {loading && !jobs.length ? <div className="llama-skeleton" aria-label="Loading activity"><div /><div /><div /></div> : !jobs.length ?
      ...
      <ol className="llama-jobs">{jobs.map(job => <li key={job.id} className="llama-job" data-state={job.state}>
        ...
        {job.state === "unknown" ? <button type="button" className="btn btn-ghost btn-sm" disabled={pending === job.id} onClick={() => action(job, "reconcile")}>{pending === job.id ? "Checking…" : "Check result"}</button> : null}
