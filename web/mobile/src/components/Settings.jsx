...
export default function Settings({ hidden, themeMode, onTheme, workspaces = [], workspacesLoaded = true }) {
  ...
  function reconnect(host, portNo, delay = 1500) {
    ...
    setMoving(true);
    setTimeout(() => { location.replace(`${scheme}://${host}:${portNo}${location.pathname}#/preferences/server`); }, delay);
  }
  ...
  async function applyPort() {
    ...
    try {
      ...
      if (res.moving) {
        ...
        const scheme = location.protocol === "http:" ? "http" : "https";
        setTimeout(() => { location.replace(`${scheme}://${host}:${res.port}${location.pathname}#/preferences`); }, 1500);
      }
    } catch (e) {
      setErr(e.message);
    }
...
function backupSteps(cfg) {
  return [
    ...
    { id: "sess", label: cfg.sessions ? "Copy agent sessions" : "Skip sessions" },
