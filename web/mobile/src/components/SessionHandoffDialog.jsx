...
export default function SessionHandoffDialog({ open, session, sourceCli, sourceName, target, onClose, onDone }) {
  ...
  useEffect(() => {
    ...
    setPreview(null); setError(""); setRunError(""); setShowBrief(false);
  }, [open, target]);
  ...
  }, [open, session, sourceCli, form.to, form.mode, form.window, form.tools, retry]);
  ...
  async function run(force) {
    ...
    if (!parsed.ok) { setRunError(parsed.error || "Check the handoff options."); return; }
    setBusy(true);
    try {
      ...
      setRunError(e?.message || "Could not continue this session. Try again.");
    } finally {
      setBusy(false);
    }
  }
        ...
        <Dialog.Content className="dlg dlg-handoff" onCloseAutoFocus={(e) => e.preventDefault()}>
          ...
          {error ? (
            <div className="handoff-error" role="alert">
              <p>{error}</p>
              <button type="button" className="btn btn-ghost btn-sm" disabled={loading || busy} onClick={() => setRetry((n) => n + 1)}>{loading ? "Retrying…" : "Retry"}</button>
            </div>
          ) : null}
