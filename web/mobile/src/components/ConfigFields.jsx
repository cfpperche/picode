...
export default function ConfigFields({ catalog, provider, model, thinking, onChange, idPrefix, row }) {
  ...
  return (
    <div className={row ? "cfg-fields cfg-row" : "cfg-fields"}>
      ...
      {provider && !models.length ? <p className="m-config-unavailable">{model ? "This model is not in the current catalog." : "No models available."} <a href="#/clis/pi/providers">Open Providers</a></p> : null}
    </div>
  );
}
