  ...
  return (
    <section className={"app-surface" + (split && !listOnly ? " app-surface-split" : "")} aria-label={title} hidden={!!hidden} ref={rootRef}>
      <header className="ft-head">
        ...
        {!listOnly ? <><span className="app-head-icon"><AppIcon name={manifest ? manifest.icon : ""} label={title} size={14} /></span>
        <h2 className="ft-title" title={title}>{title}</h2></> : null}
        ...
        {!listOnly ? <><span className="ft-spacer" />
        ...
        <div className="app-split">
          ...
          <div className="app-pane app-pane-detail" ref={detailRef}>
            {detailBlocks.length === 0 ? (
              <Blank icon={manifest ? manifest.icon : ""} label={title} text="Nothing selected — Pick an item above." />
            ) : (
