...
export default function PeerMessages({ hidden, ownerKey = "" }) {
  return <AgentClisFrame id="peer-messages-view" hidden={hidden}><CliTabs view="messages" /><Mailbox key={ownerKey} hidden={hidden} ownerKey={ownerKey} /></AgentClisFrame>;
}
    ...
    {!!m.data?.owners.length && <>
      ...
      {!m.owner ? <div className="cli-notice"><span>This conversation is no longer available.</span><a className="btn btn-ghost" href="#/clis/messages">Choose another</a></div> : <>
        ...
        {m.secret ? <section className="peer-setup" aria-label="Connect this conversation"><h4>Connect this conversation</h4><p>Use this credential only for this conversation. It is shown once.</p>
          <div className="peer-actions" data-align-row><button className="btn btn-primary" onClick={copySetup}>{copied ? "Copied" : "Copy MCP configuration"}</button><button className="btn btn-ghost" onClick={() => m.setSecret(null)}>Dismiss</button></div>
