...
test("every group id resolves to a section; every section is reachable", () => {
  ...
  const reachable = new Set([...grouped, ...MORE_ACTIONS.map(a => a.id), "packages", "providers"]);
...
test("Tools holds Integrations; Agents and connections is gone", () => {
  ...
  assert.deepEqual(MORE_GROUPS[0][1], ["pins", "clis", "automations", "apps", "llama", "integrations"]);
  assert.deepEqual(moreGroups("").find(g => g.title === "Tools").rows.map(r => r[0]), ["pins", "clis", "automations", "apps", "llama", "integrations"]);
});

test("blank query returns all groups and actions", () => {
  assert.equal(moreGroups("").length, MORE_GROUPS.length);
  assert.equal(moreGroups("").flatMap(g => g.rows).length, MORE_SECTIONS.length - 2);
...
test("Packages and Providers stay searchable inside Agent CLIs", () => {
  assert.ok(!moreGroups("").flatMap(g => g.rows).some(row => row[0] === "packages" || row[0] === "providers"));
