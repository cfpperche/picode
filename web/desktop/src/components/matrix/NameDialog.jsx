  ...
  return (
    <Dialog.Root open={!!open} onOpenChange={(o) => { if (!o) onClose(); }}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="dlg dlg-matrix-name" onCloseAutoFocus={(e) => e.preventDefault()}>
          <Dialog.Title className="dlg-title">{title}</Dialog.Title>
          <Dialog.Description className="sr-only">Name for the matrix.</Dialog.Description>
          <form noValidate onSubmit={submit}>
            ...
            />
            {error ? <p className="form-error mx-form-error" role="alert">{error}</p> : null}
