...
const Matrix = () => null;
const desktop = nativeApps({ matrix: Matrix });
const primitives = { id: "inbox", name: "Inbox", apiVersion: 1, surface: "" };
const native = { id: "matrix", name: "Matrix", apiVersion: 1, surface: "native" };

test("nativeApps: explicit assembly, frozen, ids is the gate's Set", () => {
  assert.equal(desktop.get("matrix"), Matrix);
  assert.equal(desktop.get("nope"), null);
  assert.equal(desktop.has("matrix"), true);
  assert.deepEqual([...desktop.ids], ["matrix"]);
...
test("appTile: native app registered in this shell is enabled", () => {
  assert.deepEqual(appTile(native, desktop), { ok: true, title: "Matrix" });
});
...
test("appTile: native app this build did not compile in needs a newer PiCode", () => {
  ...
  assert.match(tile.title, /no Matrix surface/);
...
test("nativeSurfaceFor: the registered component for a native manifest, null otherwise", () => {
  assert.equal(nativeSurfaceFor(native, desktop), Matrix);
