...
// state × adapter.
// | guest | bound terminal  | integrationCapable | lifecycle + launch        |
// | ----- | --------------- | ------------------ | ------------------------- |
...
test("a stopped guest offers Start terminal and Launch settings", () => {
...
test("a running guest swaps the lifecycle for restart and stop", () => {
...
test("a guest with no terminal in the list degrades to Start", () => {
  assert.deepEqual(ids(agentRowMenu({ cli: "grok", terminalId: "t9" }, { clis: [] })), ["start", "launch", "term", "rename", "remove"]);
});

test("a guest on a CLI without an adapter drops Launch settings", () => {
...
test("a guest never offers Open chat; remove is the only danger row", () => {
...
test("the guest subtitle names the CLI; Pi keeps the old wording", () => {
...
test("peer owner labels: guests name the CLI, Pi and terminals keep their wording", () => {
