...
test("the phone Work list drops launch and settings, keeps lifecycle and remove", () => {
  const running = termRowMenu({ running: true }, { surface: "phone" });
  assert.deepEqual(running.map((r) => (r.sep ? "sep" : r.id)),
    ["rename", "sep", "restart", "stop", "sep", "remove"]);
  const stopped = termRowMenu({ running: false }, { surface: "phone" });
  assert.deepEqual(stopped.map((r) => (r.sep ? "sep" : r.id)),
    ["rename", "sep", "start", "sep", "remove"]);
  const withHandoff = termRowMenu(pinned, { clis, surface: "phone" });
  assert.deepEqual(withHandoff.map((r) => (r.sep ? "sep" : r.id)),
    ["rename", "sep", "handoff", "sep", "restart", "stop", "sep", "remove"]);
  assert.equal(row(withHandoff, "launch"), undefined);
  assert.equal(row(withHandoff, "settings"), undefined);
  assert.equal(row(withHandoff, "remove").danger, true);
});
