...
test("a plain folder can be shown and copied, and has no web page", () => {
  assert.deepEqual(ids(workspaceRowMenu(FOLDER)), ["missions", "communication", "files", "|", "reveal", "copy-path", "|", "settings", "|", "remove"]);
});

test("a repository without a web remote adds only its git graph", () => {
  assert.deepEqual(ids(workspaceRowMenu(LOCAL)), ["missions", "communication", "files", "git-graph", "|", "reveal", "copy-path", "|", "settings", "|", "remove"]);
});

test("the default branch opens the repository and proposes nothing", () => {
  const rows = workspaceRowMenu(ON_MAIN);
  assert.deepEqual(ids(rows), ["missions", "communication", "files", "git-graph", "|", "reveal", "remote", "copy-path", "|", "settings", "|", "remove"]);
...
test("order moves sit above Remove, only the moves that exist", () => {
  assert.deepEqual(ids(workspaceRowMenu(FOLDER, { canMoveUp: true, canMoveDown: true })),
    ["missions", "communication", "files", "|", "reveal", "copy-path", "|", "settings", "|", "move-up", "move-down", "|", "remove"]);
