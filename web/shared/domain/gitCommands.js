// Git commands and the words that carry them — the one source for desktop,
// mobile and the git graph.
//
// ADR-0078 built three doors that write git: prepare (type into the user's
// shell), run when idle (type and submit behind the repository interlock),
// and ask the agent that owns the folder. This module composes what travels
// through them; nothing here executes anything. ADR-0096 makes the graph a
// fourth caller, which is why the helpers live here rather than being copied
// a third time.
//
// The composer is deliberately dumb about permission: it turns an action and
// its arguments into an exact command or an exact sentence. Which actions a
// surface may offer, and behind which confirmation, is the caller's decision.
import { agentsOf, displayAgentName } from "@picode/shared/domain/tree.js";

// shellQuote wraps text for a POSIX shell: single quotes, with any single
