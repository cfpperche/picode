...
test("Tools holds llama.cpp; Webhooks sit under PiCode", () => {
  ...
  assert.deepEqual(MENU_GROUPS[0][1], ["automations", "snippets", "outcomes", "history", "llama"]);
  assert.deepEqual(menuGroups("").map(g => g.title), ["Tools", "PiCode"]);
  assert.deepEqual(menuGroups("").find(g => g.title === "Tools").rows.map(r => r[0]), ["automations", "snippets", "outcomes", "history", "llama"]);
