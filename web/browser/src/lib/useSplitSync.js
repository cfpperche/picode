...
import { lineFor, previewTopFor, syncBlocks } from "@picode/shared/domain/scrollSync.js";
...
// useSplitSync ties a CodeMirror editor and a markdown preview together, the
// way VS Code's side-by-side preview does: scrolling either one scrolls the
// other to the same source line, and a double-click in the preview puts the
// editor's cursor on the line that block came from. The preview's blocks
// carry `data-line` (MarkdownDoc's `sourceLines`); `host` is any element that
// contains the preview's scroll box (`.file-preview`), which may mount late.
// `cmTick` changes whenever the editor is recreated.
export function useSplitSync({ enabled, cmRef, hostRef, cmTick }) {
  useEffect(() => {
    ...
    const previewBox = () => host.querySelector(".file-preview");
    const schedule = (fn) => {
    ...
    const onPreview = (e) => {
      const box = e.target;
      if (!(box instanceof HTMLElement) || !box.classList.contains("file-preview")) return;
