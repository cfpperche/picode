...
function WorkspaceMessages({ hidden, route }) {
  ...
  async function activate(o) {
    if (!status(o).activation) return;
    const ok = await askConfirm({title:"Activate this conversation?",message:"PiCode will send one small activation request through this idle Agent CLI. It uses one model turn and does not retry automatically.",confirmLabel:"Activate now"});
    if (ok) await m.mutate("activate", {kind:o.kind, ownerId:o.ownerId, revision:preference(o)?.revision});
  }
