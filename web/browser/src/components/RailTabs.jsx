...
import { IconFolders, IconAgent, IconTerminal, IconGrid, IconPin, IconCli, IconInbox } from "./Icons.jsx";
...
export default function RailTabs({ tab, selectTab, apps, inboxBadge, pkgUpdates, tight = false, onOpenInbox, onOpenClis }) {
  const appBadge = aggregateBadge(apps);
  const inboxLabel = inboxBadge?.count ? `Inbox, ${inboxBadge.count} need you` : inboxBadge?.dot ? "Inbox, new activity" : "Inbox";
  return (
    <nav className={"brand-tabs" + (tight ? " brand-tabs-tight" : "")} aria-label="Sidebar">
      ...
      <button type="button" className="brand-tab brand-inbox" title="Inbox" aria-label={inboxLabel} aria-current={parseRoute() === "inbox" ? "page" : undefined} onClick={() => onOpenInbox && onOpenInbox()}>
        <IconInbox size={16} />
        {inboxBadge?.count > 0 ? <span className="brand-tab-badge" aria-hidden="true">{inboxBadge.count > 99 ? "99+" : inboxBadge.count}</span> : inboxBadge?.dot ? <span className="brand-tab-dot" aria-hidden="true" /> : null}
      </button>
